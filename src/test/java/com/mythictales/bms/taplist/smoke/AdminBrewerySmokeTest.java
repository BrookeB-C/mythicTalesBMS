...
class AdminBrewerySmokeTest {
  ...
  void distributeAndReturnKegFlow() throws Exception {
    ...
    Long venueId = venues.findByBreweryId(stone.getId()).stream().findFirst().orElseThrow().getId();
    ...
    // Return
    mvc.perform(post("/admin/brewery/kegs/{id}/return", keg.getId()).with(csrf()).session(session))
        .andExpect(status().is3xxRedirection());
