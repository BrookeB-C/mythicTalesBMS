...
class AdminKegLifecycleFunctionalTest {
    ...
    void loginSessions() throws Exception {
        // Brewery admin (Stone)
        MvcResult res1 = mvc.perform(post("/login")
        ...
        // Taproom admin
        MvcResult res2 = mvc.perform(post("/login")
                .contentType(MediaType.APPLICATION_FORM_URLENCODED)
    ...
    void fullKegLifecycle_Distribute_Tap_Pour_Blow_Return_Clean() throws Exception {
        ...
        Long venueId = taps.findByTaproomId(taproomId).stream().findFirst().orElseThrow().getVenue().getId();

        // Choose a FILLED, unassigned Stone keg
        Long stoneId = users.findByUsername("stone_brewadmin").orElseThrow().getBrewery().getId();
        Keg keg = kegs.findByBreweryIdAndAssignedVenueIsNullAndStatus(stoneId, KegStatus.FILLED)
        ...
        mvc.perform(post("/admin/taproom/kegs/{id}/receive", keg.getId())
                .session(tapSession)
                .with(csrf()))
           .andExpect(status().is3xxRedirection());

        // Tap the keg onto the tap
        mvc.perform(post("/taps/{id}/tapKeg", tapId)
        ...
        mvc.perform(post("/taps/{id}/pour", tapId)
                .param("ounces", "3000")
                .with(csrf())
                .session(tapSession))
           .andExpect(status().is3xxRedirection());
        ...
        mvc.perform(post("/admin/taproom/kegs/{id}/return", keg.getId()).with(csrf()).session(tapSession))
           .andExpect(status().is3xxRedirection());
        ...
        // Reset to EMPTY
        mvc.perform(post("/admin/brewery/kegs/{id}/return", keg.getId()).with(csrf()).session(brewSession))
           .andExpect(status().is3xxRedirection());

        // Mark CLEAN
        mvc.perform(post("/admin/brewery/kegs/{id}/clean", keg.getId()).with(csrf()).session(brewSession))
           .andExpect(status().is3xxRedirection());

        // Verify visible under Kegs tab when filtering CLEAN
        mvc.perform(get("/admin/brewery").param("tab","kegs").param("status","CLEAN").session(brewSession))
           .andExpect(status().isOk())
           .andExpect(content().string(containsString(serial)))
