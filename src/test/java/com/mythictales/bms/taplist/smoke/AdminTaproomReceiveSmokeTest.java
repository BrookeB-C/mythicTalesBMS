...
class AdminTaproomReceiveSmokeTest {
    ...
    void distributeThenReceiveAppearsInTaproomLists() throws Exception {
        ...
        Long vid = taps.findByTaproomId(ua.getTaproom().getId()).stream()
                .findFirst().orElseThrow().getVenue().getId();
        ...
        // Taproom sees it in inbound list
        mvc.perform(get("/admin/taproom").session(tapSession))
        ...
        // Now appears in Available Kegs (Received)
        mvc.perform(get("/admin/taproom").session(tapSession))
           .andExpect(status().isOk())
