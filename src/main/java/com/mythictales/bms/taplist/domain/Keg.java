...
@Table(
    uniqueConstraints =
        @UniqueConstraint(
            name = "uq_keg_brewery_serial",
            columnNames = {"brewery_id", "serialNumber"}))
public class Keg {
  ...
  @ManyToOne private Venue assignedVenue; // when DISTRIBUTED/RECEIVED, assigned to venue
  ...
  public void setRemainingOunces(double remainingOunces) {
    this.remainingOunces = Math.max(0, remainingOunces);
  }

  public KegStatus getStatus() {
    return status;
  }

  public void setStatus(KegStatus status) {
    this.status = status;
  }
  ...
  public void setAssignedVenue(Venue assignedVenue) {
