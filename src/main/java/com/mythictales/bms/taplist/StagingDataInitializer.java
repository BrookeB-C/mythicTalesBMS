    ...
    CommandLineRunner initStagingData(
        ...
        return args -> {
            ...
            if (users.findByUsername("brewadmin").isEmpty()) {
                UserAccount brewadmin = new UserAccount("brewadmin", encoder.encode("stgpassword"), Role.BREWERY_ADMIN);
                brewadmin.setBrewery(br);
                users.save(brewadmin);
            }
            if (users.findByUsername("tapadmin").isEmpty()) {
                UserAccount tapadmin = new UserAccount("tapadmin", encoder.encode("stgpassword"), Role.TAPROOM_ADMIN);
                tapadmin.setTaproom(tr);
                users.save(tapadmin);
            }
            if (users.findByUsername("tapuser").isEmpty()) {
                UserAccount tapuser = new UserAccount("tapuser", encoder.encode("stgpassword"), Role.TAPROOM_USER);
